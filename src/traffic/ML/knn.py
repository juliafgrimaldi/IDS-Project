...
def train_knn(file_path):
    ...
    categorical_columns = X_resampled.select_dtypes(exclude=[np.number]).columns

    # Preencher valores ausentes (colunas numéricas)
    imputer = SimpleImputer(strategy='mean')
    X_resampled[numeric_columns] = imputer.fit_transform(X_resampled[numeric_columns])
    ...
    data_encoded = pd.DataFrame(encoder.fit_transform(X_resampled[categorical_columns]),
                                columns=encoder.get_feature_names_out(categorical_columns))
    
    data_combined = pd.concat([X_resampled[numeric_columns], data_encoded], axis=1)

    # Seleção de Atributos
    selector = SelectKBest(chi2, k=min(10, data_combined.shape[1]))
    X_selected = selector.fit_transform(data_combined, y_resampled)
